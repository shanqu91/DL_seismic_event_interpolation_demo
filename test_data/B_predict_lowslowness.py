...
# load trained model
model = load_model('../Data/trained_model_lowslowness.h5')

# prediction
Y_test = model.predict(X_test, batch_size=batch_size, verbose=1)
io.savemat('../Data/predicted_test_data_lowslowness.mat', {'Y_test':Y_test})
...
plt.title('with 8000 data excluding steep events, test data (left->input, right->predicted)')
...
# interlayer = Model(inputs=model.input, outputs=model.get_layer(layer_names[2]).output)
# features = interlayer.predict(X_test)
# print(features.shape)
# plot_feature_maps(features[0], 3)

# interlayer = Model(inputs=model.input, outputs=model.get_layer(layer_names[6]).output)
# features = interlayer.predict(X_test)
# print(features.shape)
